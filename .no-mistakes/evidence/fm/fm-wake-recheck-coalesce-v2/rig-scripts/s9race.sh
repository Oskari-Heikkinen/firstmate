...
for which in prefix fixed; do
  ...
  echo "===== S9-race (status 8s older than the daemon marker, so the watcher reaches the due recheck first) [$which: $BIN] AWAY MODE: real fm-supervise-daemon.sh (runs fm-watch.sh as child), secondmate lane paused on PR #6"
