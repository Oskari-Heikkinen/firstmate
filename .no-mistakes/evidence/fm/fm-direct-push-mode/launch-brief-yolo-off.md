...
Your steering inbox is `/tmp/fm-dp-lab.41cICM/home/state/dpush-off.inbox`; this exact path belongs to your current task even when it is outside the worktree or under the supervising firstmate home, so read and acknowledge its messages and do not reject it as another home's state.
...
1. First action: create your branch: `git checkout -b fm/dpush-off`
...
# Rules
1. Push to the default branch only through the Definition of done's landing loop, when the landing authority allows it; otherwise push only your `fm/dpush-off` branch. Never force-push anything, and never open or merge a PR.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/fm-dp-lab.41cICM/home/state/dpush-off.status' && { [ ! -e '/tmp/fm-dp-lab.41cICM/home/config/fleet-ledger' ] || '/home/oskari/.no-mistakes/worktrees/ca1b14ddd4b9/01M39AGWYZWG2QS24ZYE6GG7GK/bin/fm-fleet-ledger.sh' appended '/tmp/fm-dp-lab.41cICM/home/config' '/tmp/fm-dp-lab.41cICM/home/state/dpush-off.status' >/dev/null 2>&1 || true; }`
...
Firstmate steers you through durable message files in '/tmp/fm-dp-lab.41cICM/home/state/dpush-off.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-dp-lab.41cICM/home/state/dpush-off.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-dp-lab.41cICM/home/state/dpush-off.inbox'/NNN.msg '/tmp/fm-dp-lab.41cICM/home/state/dpush-off.inbox'/handled/`.
...
The task is complete only when committed on your branch `fm/dpush-off`.
...
Once landed, delete the task branch from origin (`git push origin --delete fm/dpush-off`) if you pushed one for ready.

Stopping at ready: run steps 1 and 2, push the tested head to your own task branch with `git push origin HEAD:refs/heads/fm/dpush-off`, append `done [at=<epoch>]: ready in branch fm/dpush-off tested on {default-branch} at {sha}`, and stop.
