...
Your steering inbox is `/tmp/fm-dpush-lab/home/state/dp-land.inbox`; this exact path belongs to your current task even when it is outside the worktree or under the supervising firstmate home, so read and acknowledge its messages and do not reject it as another home's state.
...
# Task
## Captain's intent
Add a `mul` function to calc.sh that multiplies two integers.

## Firstmate spec
Add `mul() { echo $(( $1 * $2 )); }` to calc.sh and a `check "mul 4 5" ...` case in tests/run.sh expecting 20. Keep the change minimal.
...
1. First action: create your branch: `git checkout -b fm/dp-land`
...
# Rules
1. Push to the default branch only through the Definition of done's landing loop, when the landing authority allows it; otherwise push only your `fm/dp-land` branch. Never force-push anything, and never open or merge a PR.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/fm-dpush-lab/home/state/dp-land.status' && { [ ! -e '/tmp/fm-dpush-lab/home/config/fleet-ledger' ] || '/home/oskari/.no-mistakes/worktrees/ca1b14ddd4b9/01M39535C9N8YY0760F5MNX2N8/bin/fm-fleet-ledger.sh' appended '/tmp/fm-dpush-lab/home/config' '/tmp/fm-dpush-lab/home/state/dp-land.status' >/dev/null 2>&1 || true; }`
...
Firstmate steers you through durable message files in '/tmp/fm-dpush-lab/home/state/dp-land.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-dpush-lab/home/state/dp-land.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-dpush-lab/home/state/dp-land.inbox'/NNN.msg '/tmp/fm-dpush-lab/home/state/dp-land.inbox'/handled/`.
...
The task is complete only when committed on your branch `fm/dp-land`.
...
Once landed, delete the task branch from origin (`git push origin --delete fm/dp-land`) if you pushed one for ready.

Stopping at ready: run steps 1 and 2, push the tested head to your own task branch with `git push origin HEAD:refs/heads/fm/dp-land`, append `done [at=<epoch>]: ready in branch fm/dp-land tested on {default-branch} at {sha}`, and stop.
