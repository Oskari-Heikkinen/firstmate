...
Your steering inbox is `/tmp/fm-dpush-lab/home/state/dp-ready.inbox`; this exact path belongs to your current task even when it is outside the worktree or under the supervising firstmate home, so read and acknowledge its messages and do not reject it as another home's state.
...
# Task
## Captain's intent
Add a `sub` function to calc.sh that subtracts two integers.

## Firstmate spec
Add `sub() { echo $(( $1 - $2 )); }` to calc.sh and a `check "sub 9 4" ...` case in tests/run.sh expecting 5. Keep the change minimal.
...
# Setup
You are in a disposable git worktree of dpproj, at a detached HEAD on a clean default branch.
...
1. First action: create your branch: `git checkout -b fm/dp-ready`
...
# Rules
1. Push to the default branch only through the Definition of done's landing loop, when the landing authority allows it; otherwise push only your `fm/dp-ready` branch. Never force-push anything, and never open or merge a PR.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/fm-dpush-lab/home/state/dp-ready.status' && { [ ! -e '/tmp/fm-dpush-lab/home/config/fleet-ledger' ] || '/home/oskari/.no-mistakes/worktrees/ca1b14ddd4b9/01M39535C9N8YY0760F5MNX2N8/bin/fm-fleet-ledger.sh' appended '/tmp/fm-dpush-lab/home/config' '/tmp/fm-dpush-lab/home/state/dp-ready.status' >/dev/null 2>&1 || true; }`
...
Firstmate steers you through durable message files in '/tmp/fm-dpush-lab/home/state/dp-ready.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-dpush-lab/home/state/dp-ready.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-dpush-lab/home/state/dp-ready.inbox'/NNN.msg '/tmp/fm-dpush-lab/home/state/dp-ready.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/home/oskari/.no-mistakes/worktrees/ca1b14ddd4b9/01M39535C9N8YY0760F5MNX2N8/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md`, follow `/home/oskari/.no-mistakes/worktrees/ca1b14ddd4b9/01M39535C9N8YY0760F5MNX2N8/bin/fm-ensure-agents-md.sh`'s self-governance contract in the same pass.
...
The task is complete only when committed on your branch `fm/dp-ready`.
...
Once landed, delete the task branch from origin (`git push origin --delete fm/dp-ready`) if you pushed one for ready.

Stopping at ready: run steps 1 and 2, push the tested head to your own task branch with `git push origin HEAD:refs/heads/fm/dp-ready`, append `done [at=<epoch>]: ready in branch fm/dp-ready tested on {default-branch} at {sha}`, and stop.
