...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  if [ "$KIND" = ship ]; then
    ...
    no-mistakes-prod-only)
      ...
      ;;
    *)
      echo "error: --mode must be one of no-mistakes, direct-PR, direct-push, local-only (got '$MODE')" >&2
      exit 1
      ;;
    esac
    case "$YOLO" in
...
# direct-push ranks below direct-PR: both skip the pipeline, but direct-push
# lands with no PR, so no review or pre-merge CI stands before the default branch.
