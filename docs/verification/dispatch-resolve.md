...
It proves private sentinels in the brief (a URL, a path, a file name, a token, an email, and design wording) never reach the request body, each redaction class is applied, the summary is bounded to one line of 160 characters, `--summary` is the only summary source, unknown delivery modes are dropped, a missing or fully redacted summary returns `no dispatch summary to match` without an API or quota request, and errors after the request still print the `sent:` line.
...
A live run needs a key and is not part of the suite; re-establish the table by giving each brief generic `--summary` text and running the tool with the key injected for that one command.
