...
FM_RECHECK_WARM_SECS=3300          # a due declared-wait recheck is delivered at once only while the supervisor took a turn this recently (its prompt cache is still warm); otherwise it rides along with the next wake raised for any other reason, all due rechecks always arriving together as one wake; a captain-held recheck, a paused wait whose declared until time has passed, and away mode are delivered when due
