...
The layout reference below gives each listed path a one-line role and, where one exists, its owning script; wake, watcher, away-mode, and Relay-specific state mechanics remain with those named scripts and reference sections.
...
`AGENTS.md` section 2 points here and keeps only its never-hand-edit list inline.
`LOCAL` marks gitignored, captain-private material; "never touch" marks records only their owning script may write or remove.
...
.env                 optional Relay pairing token (presence-gates AGENTS.md section 14), mail-plane credentials (schema: this doc's "Mail plane"), and typed dispatch resolution key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; this doc's "Typed dispatch resolution"); LOCAL, gitignored
...
config/claude-permission-mode  optional one-token permission posture for every Claude worker launch: absent or "bypass" keeps --dangerously-skip-permissions, "auto" launches with --permission-mode auto; LOCAL, gitignored; inherited by secondmate homes; see this doc's "Claude permission mode"
...
config/calm     Calm presentation preference shared by the Pi extension and the Claude Code mod; LOCAL, gitignored, and not inherited; see this doc's "Calm preference"
config/supervision-branch-model config/supervision-branch-effort  Pi supervision-branch model and reasoning-effort pins written by /supervision-model; LOCAL, gitignored, independently settable, and not inherited; see this doc's "Pi supervision branch model and effort"
config/startup-memory-budget     primary-authoritative per-home startup-memory budget; LOCAL, gitignored, materialized as 7,500 estimated tokens by locked primary bootstrap and inherited into secondmate homes; see this doc's "Startup memory budget"
config/stow-pass-horizon  optional presence flag opting this home in to /stow's default-off pass-count decay horizon; LOCAL, gitignored, and not inherited; see this doc's "Stow pass horizon"
...
config/wedge-defer-parked-gate  optional presence flag opting this home into the default-off deferral of a wedge escalation for a lane parked at a validation gate awaiting the supervisor's own still-open decision; LOCAL, gitignored, and not inherited; see this doc's "Parked-gate wait deferral"
...
state/               runtime records and signals; gitignored
  ...
  <id>.meta          task metadata; each producer script's header owns its exact fields and mutation contract, with this doc routing operator-facing backend and trace-context details
...
`AGENTS.md` section 4 owns the always-loaded routing precedence and load trigger, and `quota-array-dispatch` owns the intake safety rules and the completion-aware profile-array selection procedure.
...
FM_STANDING_WAITS_CEILING_SECS=86400  # longest a declared wait may go unseen by the supervisor: a held recheck wakes on its own at this age, and a due paused: recheck whose lane fingerprint (fm-classify-lib.sh declared_wait_fingerprint: status log, current-state verdict, agent liveness, task-copy HEAD and dirty bit, unread steers, and any recorded PR's state, head, and red checks) is unchanged since the wait was last shown is absorbed until then, when every such standing wait is shown together in one wake; the away-mode daemon applies the same fingerprint and ceiling
